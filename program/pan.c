...
short src_ln2 [] = {
	  0,  54,  56,  58,  59,  60,  61,  57, 
	 63,  64,  65,  66,  67,  68,  55,  70, 
	 53,  71,  53,  71,   0, };
S_F_MAP src_file2 [] = {
	{ "-", 0, 0 },
	{ "vitalflow.pml", 1, 19 },
...
short src_ln0 [] = {
	  0,  19,  21,  21,  22,  22,  23,  23, 
	 20,  25,  25,  18,  27,  18,  27,   0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "vitalflow.pml", 1, 14 },
	{ "-", 15, 16 }
...
{	int j = 0, h = now._nr_pr;
	...
	case 2:	/* Pump */
		...
		((P2 *)pptr(h))->trigger = 0;
#ifdef VAR_RANGES
		logval("Pump:trigger", ((P2 *)pptr(h))->trigger);
#endif
#ifdef HAS_CODE
		locinit2(h);
#endif
	...
	case 1:	/* Controller */
		...
		/* params: */
		/* locals: */
		((P1 *)pptr(h))->current_level = 0;
#ifdef VAR_RANGES
		logval("Controller:current_level", ((P1 *)pptr(h))->current_level);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
...
iniglobals(int calling_pid)
{
		now.pump_active = 0;
		alarm_triggered = 0;
		now.sensor_data = addqueue(calling_pid, 1, 1);
		now.pump_cmd = addqueue(calling_pid, 2, 1);
		now.glucose_level = 100;
		now.insulin_reservoir = 10;
#ifdef VAR_RANGES
		logval("pump_active", now.pump_active);
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.sensor_data) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.sensor_data) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.sensor_data) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.sensor_data) )))
			    ) return 0; break;
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.pump_cmd) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.pump_cmd) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.pump_cmd) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.pump_cmd) )))
			    ) return 0; break;
...
{	/* int i; */
	printf("global vars:\n");
	printf("	mtype  LOW:	4\n");
	printf("	mtype  NORMAL:	3\n");
	printf("	mtype  HIGH:	2\n");
	printf("	mtype  CRITICAL_HIGH:	1\n");
	printf("	mtype  IDLE:	8\n");
	printf("	mtype  PUMPING:	7\n");
	printf("	mtype  ALARM:	6\n");
	printf("	mtype  EMPTY:	5\n");
	printf("	int    glucose_level:	%d\n", now.glucose_level);
	printf("	int    insulin_reservoir:	%d\n", now.insulin_reservoir);
	printf("	bit    pump_active:	%d\n", now.pump_active);
	printf("	chan sensor_data (=%d):	len %d:\t", now.sensor_data, q_len(now.sensor_data));
	c_chandump(now.sensor_data);
	printf("	chan pump_cmd (=%d):	len %d:\t", now.pump_cmd, q_len(now.pump_cmd));
	c_chandump(now.pump_cmd);
}
void
...
{	/* int i; */
	...
	case 3:
		...
		break;
	case 2:
		printf("local vars proc %d (Pump):\n", pid);
	printf("	bit    trigger:	%d\n", ((P2 *)pptr(pid))->trigger);
		break;
	case 1:
		printf("local vars proc %d (Controller):\n", pid);
	printf("	int    current_level:	%d\n", ((P1 *)pptr(pid))->current_level);
		break;
	case 0:
		/* none */
		break;
...
printm(int x, char *s)
{
	...
	switch (x) {
	case 1: Printf("CRITICAL_HIGH"); return;
	case 2: Printf("HIGH"); return;
	case 3: Printf("NORMAL"); return;
	case 4: Printf("LOW"); return;
	case 5: Printf("EMPTY"); return;
	case 6: Printf("ALARM"); return;
	case 7: Printf("PUMPING"); return;
